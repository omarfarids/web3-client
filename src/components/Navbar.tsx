      ...
      <div className='flex justify-between gap-5'>
        <div className='relative'>
          ...
          {['Market','Exchange','Tutorials','Wallets'].map((element,index)=>{
            return <li key={index} className='w-32 cursor-pointer text-center hover:text-black hover:bg-purple-100/75'>{element}</li>
          })}
        </ul>
        </div>
        <ul className='sm:flex justify-between gap-3 items-center hidden text-white'>
          {['Market','Exchange','Tutorials','Wallets'].map((element,index)=>{
